...
<body ng-app="m" ng-init='index = "员工考勤管理"'>
...
<div ng-controller="c" class="container" style="width: 98%;">
               ...
               <!--  <label>
                ...
                <!-- <form class="form-inline pull-right" ng-submit="page.refreshTo(1)">
                    <input class="form-control m-l" type="text"
                           placeholder="打卡类型/用户名/金额" ng-model="search">
    ...
    <div entity-modal="modal-edit" title="编辑 打卡记录" action="edit" entity="entityEdit">
        <entity-modal-body>
            <div entity-edit-text="checkItem" type="text" title="打卡类型" entity="entityEdit.entity"
                 e="entityEdit" vld="entityEdit.validate"></div>
            <div entity-edit-text="remark" type="text" title="备注" entity="entityEdit.entity"
                 e="entityEdit" vld="entityEdit.validate"></div>
        </entity-modal-body>
    </div>
    <div entity-modal="modal-del" title="删除 打卡记录" action="delete" entity="entityDel">
        <entity-modal-body>
            <h4>确认删除该条打卡记录？</h4>
        </entity-modal-body>
    </div>
...
<script>
    ...
    var c = m.controller("c", function ($rootScope,$scope,page,ajax,$filter,entity,uploader) {
        ...
        $scope.page = page.page(function (current, size) {
            ajax.ajax("/check/getCheckPageList", "GET", {
                    current: current,
                    size: size,
                    search: $scope.search
        ...
        $scope.entity = entity.getEntity(
            {	 
               ...
               console.log("submited!");
                ...
                ajax.ajax("/check/updateOneCheck", "GET", {
                        checkItem: $scope.entity.entity.salaryItem,
        ...
        $scope.entityEdit = entity.getEntity(
            {
                ...
                console.log("opened!");
                $scope.entityEdit.entity.checkItem = $scope.editObj.checkItem;
                $scope.entityEdit.entity.remark = $scope.editObj.remark;
            }, function () {
                ajax.ajax("/check/updateOneCheck", "GET", {
                    id: $scope.editObj.id,
                    checkItem:$scope.entityEdit.entity.checkItem,
                    remark:$scope.entityEdit.entity.remark
                }).success(function (data) {
                    console.log(data);
                    if(data.success){
            ...
            $scope.entityDel = entity.getEntity(
            {
               id:0
                ...
                ajax.ajax("/check/deleteOneCheck", "GET", {
                    id: $scope.delObj.id,
                }).success(function (data) {
                    console.log(data);
                    if(data.success){
